...
export function ConnectionProvider({ children }) {
    const [endpoint, setEndpoint] = useLocalStorageState('endpoint', clusterApiUrl('mainnet-beta'));
    const connection = useMemo(() => new Connection(endpoint, 'recent'), [
        endpoint,
    ]);
    return (React.createElement(ConnectionContext.Provider, { value: { endpoint, setEndpoint, connection } }, children));
}
